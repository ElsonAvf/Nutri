...
class Fit {
  // Body Mass Index
  BMI(weight, height) {
    return parseFloat(((weight / (height ** 2)) * 10000).toFixed(1))
  }
  // Lean Body Mass
  //Boer formula LMB
  ...
  originalHarrisBenedictBMR(weight, height, age, gender) {
    if(gender === 'm') {
      ...
      return Math.round(655.1 + (9.56 * weight) + (1.8 * height) - (4.67 * age))
    }
  }
  ...
  miffinStJeorBMR(weight, height, age, gender) {
    if(gender === 'm') {
  ...
  katchMcArdleBMR(weight, height, gender, LBM) {
    if(typeof LBM === 'number') {
      return Math.round(370 + (21.6 * LBM))
    } else if(typeof LBM === 'function') {
      return Math.round(370 + (21.6 * LBM(weight, height, gender)))
    }
  }
  ...
  TDEE(BMR) {
    return {
      ...
      extremely: Math.round(1.9 * BMR)
    }
  }
  // Ideal Body Weight
  IBW(height, gender) {
    return {
      peterson: this.petersonIBW(height),
      robinson: this.robinsonIBW(height, gender),
      devine: this.devineIBW(height, gender),
      miller: this.millerIBW(height, gender),
      hamwi: this.hamwiIBW(height, gender),
      broca: this.brocaIBW(height, gender),
      lemmens: this.lemmensIBW(height),
      bmi: this.BMIRangeIBW(height)
    }
  }
  // Peterson formula for IBW
  petersonIBW(height) {
    // Chosen BMI target 22 as it is uses in many calculators
    return Math.round(2.2 * 22 + 3.5 * 22 * ((height / 100) - 1.5)) // Height in meters
  }
  ...
  lemmensIBW(height) {
    // Ideal BMI = 22
    return Math.round(22 * ((height / 100) ** 2)) // Height in meters
  }
  BMIRangeIBW(height) {
    return {
